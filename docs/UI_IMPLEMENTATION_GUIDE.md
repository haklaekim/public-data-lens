...
| 7 | **Lens** | 렌즈 4종이 무엇에 답하는지 | 정적 + `/api/datasets/{id}` 실물 링크 |
...
`LensNavigation`은 렌즈만 담는다. 기술 뷰는 별도 접힘 영역으로 분리한다.

```
렌즈:     Overview | Structure | Evidence | Possible Uses*
접힘:     원본 데이터 ▸ (정규화 / 원본 CSV / JSON-LD)
                                          * REST 경로 추가 후
```
...
최소 처리: 접힘 상태 기본, 필드명 검색, 복사 버튼, `view` 별 1줄 설명.
...
| **L2** | 속성 전체·설명·기관 기재 한계·컬럼 구조·예시값·수집 상태 | 렌즈 진입 시 |
| **L3** | 규칙 버전·색인 버전·관측 ID·정규화 원본·원본 CSV·JSON-LD | 접힘 |
...
현상태: 5개 파일이 startsWith('본 결과는') 필터를 복제.
        서버 DISCLAIMER 문안에 문자열로 결합된 상태 → 단일화(§12)
```
...
| ADR | `decisions/001-css-토큰-분리.md` |
