...
AIRD(AI-Ready Data) 표준안 실증 프로젝트.
...
| `apps/server/datanav/spec/tool-schemas-v1.4.0.json` | **enum·필드의 정본** |
...
8. **`api.js`의 8개 메서드 시그니처를 바꾸지 않는다.**
...
```
cd apps/server && python -m pytest -q          # 13개 파일
cd apps/web && VITE_SURFACE=core npm run build # 프로덕션 표면
```

`apps/web`에는 현재 테스트가 없다. CSS·컴포넌트를 변경하기 전에 Playwright 시각
베이스라인을 확보한다.

## 표면 분기

`VITE_SURFACE` = `core`(프로덕션, MCP 동반 웹) | `concierge` | `all`(로컬 기본).
컨시어지 컴포넌트는 공개 스냅샷에 부재하며 `import.meta.glob`이 빈 객체를 반환해
표면 자체가 사라진다.
...
- **`decisions/`** — 설계 판단이 있었으면 ADR. `NNN-제목.md`.
  형식: 맥락 / 결정 / 기각한 대안과 이유 / 영향
